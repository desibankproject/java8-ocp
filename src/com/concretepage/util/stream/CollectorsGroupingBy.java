...
public class CollectorsGroupingBy {
    public static void main(String[] args) {
        ...
        Map<String, List<String>> stdByClass = list.stream()
                    .collect(Collectors.groupingBy(Student::getClassName,Collectors.mapping(Student::getName, Collectors.toList())));
        System.out.println(stdByClass);
        
      //  Map<String, List<Student>> stdByClass.forEach((k,v)->System.out.println("Key:"+k+"  "+  ((List<Student>)v).stream().map(m->m.getName()).collect(Collectors.joining(","))));
